...
import { useAuth, User } from "@/Context/userAuth";
...
export default function Navbar() {
  ...
  const { user, setUser, addUser} = useAuth();
  ...
  useEffect(() => {
    const addLoggedInUser = (user: User) => {
      addUser(user); // Add the logged-in user to your user context
    };

    if (session) {
      addLoggedInUser(session.user as User);
    }
  }, [session, addUser]);
